...
function makeLevel(levelData)
    ...
    for j = 2, levelData.cols + 1 do
        ...
        grid.tiles[levelData.rows + 2][j] =
            Tile:new(GameConstants.Tile.BRICK_BOT)
    end
    ...
    for i = 1, levelData.rows + 1 do
        ...
        grid.tiles[i][levelData.cols + 2] =
            Tile:new(GameConstants.Tile.BRICK_RIGHT)
    end
...
function makeTile(id) return Tile:new(TILE_IDS[id]) end
